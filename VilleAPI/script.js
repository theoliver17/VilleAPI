...
function search() {
  ...
  if (searchValue === "") {
    ...
    errorMsg.innerHTML = "Aller la met quelque chose !";
    resultsDiv.appendChild(errorMsg);
    return 0;
  }

  fetch(
    `https://geo.api.gouv.fr/communes?nom=${searchValue}&fields=code,codesPostaux,nom,region,surface,departement,population,departement&boost=population&limit=10`
  )
    .then((response) => {
